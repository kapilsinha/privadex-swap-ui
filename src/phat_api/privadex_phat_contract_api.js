...
async function contractApi(api, pruntimeUrl, contract) {
    ...
    const phala = await phala_create({ api: newApi, baseURL: pruntimeUrl, contractId: contract.address, autoDeposit: true });
...
export class PrivaDexAPI {
    ...
    static async initialize() {
        ...
        const contractPrivadex = loadPrivadexContractFile('0xcd9cb06dd6c9d8613f8ceb906b72903e365991ce6a0149bfc0c54816c8d68569');
    ...
    async escrowEthAddress() {
        const escrowAddress = await this.#contractApi.query.getEscrowEthAccountAddress(this.#certSudo, {},);
        console.log("PrivaDEX escrow =", escrowAddress, escrowAddress.output.asOk.toString());
        return escrowAddress.output.asOk.toString();
    }

    async quote(srcChain, destChain, srcTokenEncoded, destTokenEncoded, amountIn) {
        const quote = await this.#contractApi.query.quote(
            this.#certSudo,
            {},
            srcChain,
            destChain,
        ...
        // console.log("PrivaDEX quote =", amountIn.toString(), quote);
        // console.log(quote.output.asOk.toBigInt());
        return quote.output.asOk.toBigInt(); 
    }
    ...
    async startSwap(
        ...
        let uuid = '0x' + exec_plan_uuid.output.asOk.map(x => x.toString(16).padStart(2, '0')).join('');
